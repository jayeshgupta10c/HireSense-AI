...
1.  **Clone and Enter**:
    ...
    cd HireSense-AI
    ```
2.  **Environment Variables & Python Version**:
    - **CRITICAL (Render)**: Set the environment variable `PYTHON_VERSION` to `3.10.13` in your Render dashboard settings.
    - `MONGO_URI` = *(Paste your exact MongoDB string from Phase 1)*
    - `JWT_SECRET` = `hiresense_production_secure_key_2026`
    - `JWT_ALGORITHM` = `HS256`
3.  **Launch**:
    ```bash
